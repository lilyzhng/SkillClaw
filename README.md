...
```
              ...
              ▼                  ▼          ▼
┌──────────────────────┐  ┌──────────────┐  ┌─────────────────┐
│ 🧪 Solver Agent(s)   │  │ 🤖 PR Agent  │  │ 🔍 Oversight    │
│ (Claude Sonnet)      │  │ (no LLM)     │  │ (Claude Opus)   │
│                      │  │              │  │                 │
│ 1. UNDERSTAND task   │  │ Renders traj │  │ Reviews code:   │
│ 2. STUDY patterns    │  │ → PR body    │  │ - no hardcoded  │
│ 3. WRITE solve()     │  │ + GIF        │  │   positions     │
│ 4. EXECUTE sim_exec  │  │ + git commit │  │ - uses helpers  │
│ 5. DEBUG via state   │  │ + gh pr      │  │ - error handle  │
│ 6. SAVE skill        │  │              │  │ + visual check  │
│                      │  │ Tools:       │  │   (demo GIF)    │
│ Tools:               │  │  git, gh,    │  │                 │
│  bash(), sim_exec()  │  │  ffmpeg      │  │ Tools:          │
│  ▼            ▼      │  │              │  │  gh pr diff,    │
│ Local      GPU VM    │  │              │  │  gh pr review,  │
│ Files      ManiSkill │  │              │  │  gh pr merge    │
└──────────┬───────────┘  └──────┬───────┘  └────────┬────────┘
           │                     ▲                    ▲
           │    pr_queue         │   review_queue     │
           └─────────────────────┘────────────────────┘
```
