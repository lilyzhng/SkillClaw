...
def solve(env, planner):
    ...
    Strategy:
    1. Open gripper
    2. Approach from the side opposite to pull direction
    3. Move to grasp position with small offset
    4. Close gripper to grasp the cube
    5. Pull to goal position maintaining y and z coordinates
    """
    env = env.unwrapped
    ...
    # -------------------------------------------------------------------------- #
    # Open gripper and approach from positive x side (object is at x=0.0765, 
    # goal is at x=-0.1235, so we approach from the right side)
    # -------------------------------------------------------------------------- #
